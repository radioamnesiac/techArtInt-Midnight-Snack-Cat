...
public class GameManager : MonoBehaviour
{
    ...
    public Board board;
    public GameObject[] gameplayUIElements, gameOverUIElements, nextRoundUI;
    public GameObject additionUI, shuffleUI;
    public GameObject nextLunch;
  ...
  */
    ...
    */
    void Timer()
    {
        if (timer <= 60)
        {
            anim.SetTrigger("halfTimeBoil");
        }
        if (timer <= 30)
        {
            anim.SetTrigger("quartTimeBoil");
        }
        if (timer <= 0 && !gameover)
        {
            GameOver();
    ...
    */
    public void AddScore(int sequenceCount, int combo = 1)
    {
        ...
        {
            nextLunch.GetComponent<Animator>().SetBool("nextLunch", true);
